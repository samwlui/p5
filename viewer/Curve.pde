...
class Curve {
  ...
  int cn = 0;                           //current number of control points
  pt [] P = new pt[5000];            //  array of points
  pt [] controlP = new pt[5000];        // array of control points
...
//    };
  ...
  void resetPoints(float r) {
    ...
    void rotatePoints(float r) {
    //println(">>n="+n); 
    for (int i=0; i<n; i++) {
      P[i].x=r*cos(TWO_PI/n*i); 
      P[i].z=r*sin(TWO_PI/n*i);
    };
  } // init the points to be on a circle
  
  void rotatePointsAboutY(float r){
   for (int i=1; i<n-1; i++) {
     P[i].rotateAboutY(radians(r));
    };
  }
  ...
  void dragAllButFandL(vec V) {
    for (int i=1; i<n-1; i++) P[i].add(V);
  }
  ...
  Curve makeFrom(Curve C, int v) {
    ...
    for (float t=0; t<=1.001; t+=.1/v) 
    {
      ...
      //append(C.interpolate2(t));
    }
    return this;
  }
  
    Curve makeFrom2(Curve C, int v) {
    empty(); 
    for (float t=0; t<=1.001; t+=.1/v) 
    {
    //  append(C.interpolate(t)); 
  ...
  pt interpolate2(float t){
        return D(P[4], P[5], P[6], P[7], P[8], t);
  }
  ...
  //show individual pt at the index
    Curve showSample(int index, float r) {
    show(P[index], r); 
    return this;
  }  // fast draw of edges
    Curve showC(float r){
 ...
 pt[] subDivide(){
  ...
  for(int i = 1; i < n-2; i++){
    ...
    pt h1 =  D(P[a], P[b], P[c], 0.75);
    pt h2 = D(P[d], P[c], P[b], 0.75);
    pt midpoint = P(h1,h2);
    temp[i*2+1] = midpoint;
 ...
 temp[n*2-4] = P[n-2];
 temp[n*2-3] =  D(P[n-3], P[n-2], P[n-1], 0.75);
 temp[n*2-2] =  P[n-1];
...
return P; 
  ...
  Curve convex(){
    ArrayList<pt> convexCurve = new ArrayList<pt>();    
    convexCurve.add(P[0]);
    int newN = n;
    for(int i=1; i<n-1; i++){
      if(leftTurn(P[i-1],P[i],P[i+1])){
          newN--;
        }
      else{ convexCurve.add(P[i]); }
    }
    convexCurve.add(P[n-1]);
    for(int i=0; i<newN; i++){
      P[i] = convexCurve.get(i); 
    }
    n = newN;
