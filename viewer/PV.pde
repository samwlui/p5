...
class pt { float x=0,y=0,z=0; 
   ...
   void rotateAboutY(float r){rotateY(radians(r));}
...
boolean leftTurn(pt A, pt B, pt C) { return ((B.x - A.x) * (C.y - A.y) - (B.y - A.y) * (C.x - A.x)) > 0;}
...
//void arrayDelete(int currentIndex, int amount, particle[] pa){
//  for(int i = currentIndex; i< amount-1; i++){
//     pa[i] = pa[arrayNext(i)]);
//     amount--;
//    currentIndex = arrayPrev(currentIndex);  
//  }
//   
//}
//  void delete() { 
...
//edge functions
Edge edge(pt p, pt p1) { 
  return new Edge(p, p1); 
}
