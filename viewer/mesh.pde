...
vec[] Nt = new vec [maxnt];                // triangles normals
 ...
 HashMap<Integer, List<Edge>> eMap = new HashMap<Integer, List<Edge>>();
 ArrayList<Edge> edges = new ArrayList<Edge>(); 
 Mapping[] mapToOthers = new Mapping[3];
 
 void addEdge(pt A, pt B, int nt){
  int w = this.edges.indexOf(new Edge(A, B));
  if (w != -1) {
    this.edges.get(w).addTri(nt);
    Edge f = this.edges.get(w);
  } else {
    this.edges.add(new Edge(A,B,nt));
  }
}

void map(int i, Mesh otherMesh) {
  mapToOthers[i] = new Mapping(this, otherMesh);
}
...
void showMorph(float t) {
  Mapping map = mapToOthers[0];
  for (int i = 0; i < nt; i++) { 
    List<pt> vertices = map.ftov.get(i);
    for (pt cur : vertices) {
      fill(green);
      noStroke();
      beginShape();
        vertex(P(G[V[3 * i + 0]],t,cur));
        vertex(P(G[V[3 * i + 1]],t,cur));
        vertex(P(G[V[3 * i + 2]],t,cur));
      endShape();
    }
  }

  for (int i = 0; i < nc; i++) {
    List<Integer> tr = map.vtof.get(i);
    for (Integer p : tr) {
      fill(red);
      noStroke();
      beginShape();
        vertex(P(G[V[i]],t,P(map.get2MV(3 * p + 0))));
        vertex(P(G[V[i]],t,P(map.get2MV(3 * p + 1))));
        vertex(P(G[V[i]],t,P(map.get2MV(3 * p + 2))));
      endShape();
    }
  }
  
  for (Entry<Edge, List<Edge>> e : map.etoe.entrySet()) {
      Edge e1 = e.getKey();
      ...
      for (Edge e2 : edges) {
        ...
        noStroke();
        beginShape();
          vertex(P(e1.B,t,e2.B));
          vertex(P(e1.A,t,e2.B));
          vertex(P(e1.A,t,e2.A));
          vertex(P(e1.B,t,e2.A));
        endShape();
      }
  }
  ...
  void buildEMap() {
    ...
    eMap.clear();
    for (int i=0; i<nc; i++) {
      if (!eMap.containsKey(v(i))) {
        //println(v(i));
        eMap.put(v(i), new ArrayList<Edge>());
      }
      eMap.get(v(i)).add(edge(g(i), g(n(i))));
      eMap.get(v(i)).add(edge(g(i), g(p(i))));
    }
    //println(eMap);
  }

// void revolve(RotateSweep s) {
//  Curve current;
//  int faces = s.k;
//  if (faces == 0) return;
//
...
//  ==================================== INIT, CREATE, COPY ====================================
 ...
 Mesh empty() {nv=0; nt=0; nc=0;  eMap = new HashMap<Integer, List<Edge>>(); return this;}
...
void makeDChain (float w, int m) { // make a chain of size 2w wiht m elements
 ...
 void addTriangle(int i, int j, int k) {V[nc++]=i; V[nc++]=j; V[nc++]=k; visible[nt++]=true; addEdge(G[i],G[j], nt); addEdge(G[j],G[k], nt); addEdge(G[i],G[k], nt);} // adds a triangle
 void addTriangle(int i, int j, int k, int m) {V[nc++]=i; V[nc++]=j; V[nc++]=k; tm[nt]=m; visible[nt++]=true; addEdge(G[i],G[j], nt); addEdge(G[j],G[k], nt); addEdge(G[i],G[k], nt);} // adds a triangle
